...
class Uav:
    ...
    def landingSequence(self):
        ...
        loiter_alt = 20
        loiter_rad = 50
        loiter_lat, loiter_long = new_waypoint(home_lat, home_long, start_land_dist, self.config_data['bearing'] - 180)

        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_LOITER_TO_ALT, 0, 1, 0, loiter_rad, 0, 0, loiter_lat, loiter_long, loiter_alt))
        x, y = new_waypoint(home_lat, home_long, 50, self.config_data['bearing'])
        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_NAV_LAND, 0, 1, 0, 0, 0, 0, x, y, 0))
        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_DO_SET_SERVO, 0, 1, self.config_data['servoNo'], self.config_data['PAYLOAD_CLOSE_PWM_VALUE'], 0, 0, 0, 0, 0))

    def open_servo_wp(self):
        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_DO_SET_SERVO, 0, 1, self.config_data['servoNo'], self.config_data['PAYLOAD_OPEN_PWM_VALUE'], 0, 0, 0, 0, 0))

    def add_delay_wp(self):
        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_CONDITION_DELAY, 0, 1, 5, 0, 0, 0, 0, 0, 0))
    ...
    def add_home_wp(self):
        ...
        self.wp.add(mavutil.mavlink.MAVLink_mission_item_message(self.vehicle.target_system, self.vehicle.target_component, 0, mavutil.mavlink.MAV_FRAME_GLOBAL_RELATIVE_ALT, mavutil.mavlink.MAV_CMD_DO_SET_HOME, 0, 1, 0, 0, 0, 0, home[0], home[1], 0))
