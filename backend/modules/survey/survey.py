from modules.utils import get_bearing_2_points, new_waypoint, get_dist_2_points 
from .rect_points import RectPoints

def generateSurveyFromList(search_list: list[list[float]], spacing, planeLocation, survey_alt) -> list[list[float]]:
    if len(search_list) != 4:
        raise ValueError("Search grid must have 4 points to form a rectangle")
        
    searchRec = RectPoints(*search_list)
    return generateSurveyFromRect(searchRec, spacing, planeLocation, survey_alt)

def generateSurveyFromRect(search_rect: RectPoints, spacing, planeLocation, survey_alt) -> list[list[float]]:
    points = []
    ...
    while uncoveredDistance > 0:
        points.append([lastPoint[0], lastPoint[1], survey_alt])
